...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            ...
            img_emb_mu, img_emb_var, img_emb, cap_emb_mu, cap_emb_var, cap_emb = model.forward_emb(images, captions, lengths, image_lengths=image_lengths)
        else:
            # img_emb, cap_emb = model.forward_emb(images, captions, lengths)
            img_emb_mu ,img_emb_var,img_emb,cap_emb_mu,cap_emb_var,cap_emb = model.forward_emb(images, captions, lengths)
        ...
        if img_embs is None:
            ...
            img_emb_mus = np.zeros((len(data_loader.dataset), img_emb.size(-1)))
            img_emb_vars = np.zeros((len(data_loader.dataset), img_emb.size(-1)))
            cap_emb_mus = np.zeros((len(data_loader.dataset), cap_emb.size(-1)))
            cap_emb_vars = np.zeros((len(data_loader.dataset), cap_emb.size(-1)))
        ...
        img_emb_mus[ids] = img_emb_mu.data.cpu().numpy().copy()
        img_emb_vars[ids] = img_emb_var.data.cpu().numpy().copy()
        cap_emb_mus[ids, :] = cap_emb_mu.data.cpu().numpy().copy()
        cap_emb_vars[ids, :] = cap_emb_var.data.cpu().numpy().copy()

        # measure accuracy and record loss
        # model.forward_loss(img_emb, cap_emb)
        model.forward_loss(img_emb_mu, img_emb_var, img_emb, cap_emb_mu, cap_emb_var, cap_emb)
    ...
    return img_emb_mus, img_emb_vars, img_embs, cap_emb_mus, cap_emb_vars, cap_embs
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    with torch.no_grad():
        if opt.precomp_enc_type == 'basic':
            img_emb_mus ,img_emb_vars,img_embs,cap_emb_mus,cap_emb_vars,cap_embs = encode_data(model, data_loader)
        else:
            img_emb_mus ,img_emb_vars,img_embs,cap_emb_mus,cap_emb_vars,cap_embs = encode_data(model, data_loader, backbone=True)
...
def compute_sim(images, captions):
    ...
    similarities = similarities.data.cpu().numpy().copy()
    # sim = np.matmul(images[:, 0, :], np.matrix.transpose(captions[:, 0, :]))
    # sim2 = np.matmul(images[:, 1, :], np.matrix.transpose(captions[:, 1, :]))
    # sim3 = np.matmul(images[:, 2, :], np.matrix.transpose(captions[:, 2, :]))
    # sim4 = np.matmul(images[:, 3, :], np.matrix.transpose(captions[:, 3, :]))
